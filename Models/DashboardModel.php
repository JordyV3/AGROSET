<?php 
	class DashboardModel extends Mysql
	{
		...
		public function gastoPorCliente(){
			$rolid = $_SESSION['userData']['idrol'];
			$idUser = $_SESSION['userData']['idpersona'];
			$where = "";
			if($rolid == RCLIENTES ){
			...
			$sql = "SELECT SUM(monto) as total FROM pedido ".$where;
			$request = $this->select($sql);
			$total = $request['total'];
			return $total;
		}
		...
		public function lastOrders1(){
			$rolid = $_SESSION['userData']['idrol'];
			$idUser = $_SESSION['userData']['idpersona'];
			$where = "";
			if($rolid == RCLIENTES ){
				$where = " WHERE pe.personaid = ".$idUser;
			}
		
			$sql = "SELECT pe.idpedido, p.nombres AS nombre, p.apellidos AS apellido,
			pr.nombre AS producto,
			pe.monto AS monto,
			pe.status AS estado
				FROM pedido pe
				LEFT JOIN persona p ON pe.personaid = p.idpersona
				LEFT JOIN detalle_pedido dp ON pe.idpedido = dp.pedidoid
				LEFT JOIN producto pr ON dp.productoid = pr.idproducto
				$where
				ORDER BY pe.idpedido DESC LIMIT 10";
			$request = $this->select_all($sql);
			return $request;
		}
		...
		public function selectProductosPorCategoria1(int $anio, int $mes){
			...
			$sql = "SELECT pe.personaid, c.nombre AS categoria, COUNT(*) AS cantidad
					FROM detalle_pedido dp
					JOIN pedido pe ON dp.pedidoid = pe.idpedido
					JOIN producto pr ON dp.productoid = pr.idproducto
					JOIN categoria c ON pr.categoriaid = c.idcategoria
					WHERE MONTH(pe.fecha) = $mes AND YEAR(pe.fecha) = $anio
					GROUP BY pe.personaid, c.nombre";
			$productosPorCategoria = $this->select_all($sql);
			$meses = Meses();
			$arrData = array('anio' => $anio, 'mes' => $meses[intval($mes-1)], 'productosPorCategoria' => $productosPorCategoria);
			return $arrData;
		}


		public function selectProductosPorCategoria(int $anio, int $mes) {
			$rolid = $_SESSION['userData']['idrol'];
			$idUser = $_SESSION['userData']['idpersona'];
			
			$where = "";
			if ($rolid == RCLIENTES) {
				$where = " AND pe.personaid = $idUser";
			}
			...
			$sql = "SELECT pe.personaid, c.nombre AS categoria, COUNT(*) AS cantidad
					...
					JOIN producto pr ON dp.productoid = pr.idproducto
					JOIN categoria c ON pr.categoriaid = c.idcategoria
					WHERE YEAR(pe.fecha) = $anio AND MONTH(pe.fecha) = $mes $where
					GROUP BY pe.personaid, c.nombre";
